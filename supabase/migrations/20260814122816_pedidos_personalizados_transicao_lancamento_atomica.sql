-- Substitui a RPC vigente para persistir o número de lançamento e concluir
-- RASCUNHO -> VENDA FECHADA na mesma transação PostgreSQL.
-- PostgREST/Supabase não suporta funções RPC sobrecarregadas com segurança.
drop function public.transicionar_pedido_personalizado(
...
begin
  ...
  v_numero_lancamento := case
    when v_pedido.status = 'RASCUNHO' and p_status_destino = 'VENDA FECHADA'
      then coalesce(nullif(btrim(p_numero_lancamento), ''), v_pedido.numero_lancamento)
    else v_pedido.numero_lancamento
  end;

  if p_numero_lancamento is not null
     and not (v_pedido.status = 'RASCUNHO' and p_status_destino = 'VENDA FECHADA')
  then raise exception 'NUMERO_LANCAMENTO_NAO_PERMITIDO' using errcode = '22023'; end if;

  if v_pedido.status = 'RASCUNHO'
     and p_status_destino = 'VENDA FECHADA'
     and (v_numero_lancamento is null or v_numero_lancamento !~ '^[0-9]{1,6}$')
  ...
  set status = p_status_destino,
      numero_lancamento = case
        when v_pedido.status = 'RASCUNHO' and p_status_destino = 'VENDA FECHADA'
          then v_numero_lancamento
        else pedido.numero_lancamento
      end,
...
revoke all on function public.transicionar_pedido_personalizado(
  uuid, integer, uuid, text, text, text, date, text, date, text
