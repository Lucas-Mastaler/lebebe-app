...
function isOurDeslocEvent_(evt){
  const t = (evt.summary || evt.title || '').toUpperCase();
  return /DESLOCAMENTO/.test(t); // evita loop por editar o próprio deslocamento
}
...
function scanChangedSlots_() {
  ...
  Object.keys(newMap).forEach(key=>{
    if (newMap[key] !== prevMap[key]) changed.push(slots[key]);
  });

  // 5) Força recálculo se não houver evento de deslocamento criado mas houver endereço
  const changedKey = new Set(changed.map(o => `${toYmd(o.dia)}|${o.team}`));
  ...
  Object.keys(slots).forEach(key => {
    ...
    changed.push({ dia: d, team: pack.team });
    changedKey.add(key);
  });

  PropertiesService.getScriptProperties().setProperty(PROP_CAL_SIG_MAP, JSON.stringify(newMap));
  return changed;
}
...
function gerarEventosDeslocamento() {
  const changes = scanChangedSlots_(); // só o que realmente mudou na janela D+2..D+100
  ...
  changes.forEach(({dia, team}) => {
    try {
      ...
      Logger.log(`⚠️ Erro ao recalc ${formatDatePt(dia)} ${team}: ${e.message}`);
    }
  });
}
