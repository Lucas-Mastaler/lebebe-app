...
export default function ConferenciaPage() {
  ...
  const filtered = baseItems
    .filter(item => {
      ...
      const itemWithRefs = item as { refs_display?: string };
      const refs = (itemWithRefs.refs_display?.toLowerCase()) || ''
      ...
      <div className="flex items-start justify-between mb-2">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5 flex-wrap">
            <p className="text-xs font-mono text-slate-500">{(item as { refs_display?: string }).refs_display || item.nfe_item?.codigo_produto}</p>
...
function LocalModal({
  ...
  async function handleSave() {
    // Se refs foram alteradas, mostrar confirmação antes de salvar
    if (refChanged) {
      setShowConfirm(true)
      return
    }

    // Se refs não foram alteradas, salvar local direto
    await executeSaveLocalOnly()
  }

  async function executeSaveLocalOnly() {
    setSaving(true)
    try {
      ...
      if (!res.ok) {
        ...
        console.error('Erro ao salvar local:', errorText)
  ...
  async function executeSaveWithRefs() {
    setSaving(true)
    setShowConfirm(false)
    try {
      ...
      // 2. Salvar refs
      const refsSaved = await executeSaveRefs()
      if (!refsSaved) {
        // Falha ao salvar refs - não fechar modal
        setSaving(false)
        return
      }

      // 3. Tudo salvo com sucesso
      toast.success('Local salvo com sucesso')
      await onSave(corredor, nivel, prateleira, volumesPorItem)
    } catch (err) {
      console.error('Erro ao salvar local:', err)
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40" onClick={onClose}>
      <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full max-w-md p-6 shadow-xl" onClick={e => e.stopPropagation()}>
        ...
        {/* Refs edit (sensíveis) */}
        <div className="bg-amber-50/40 border border-amber-200 rounded-lg p-3 mb-6">
          <div className="flex items-center gap-1.5 mb-2">
            <AlertTriangle className="w-3.5 h-3.5 text-amber-600" />
            <span className="text-xs font-semibold text-amber-700">Refs do cadastro SKU (sensível)</span>
          </div>
          <div className="flex gap-2 items-end">
            <div className="flex-1 min-w-0">
              <label className="text-xs text-amber-600 block mb-1">Ref meia</label>
              <input
                type="text"
                value={editRefMeia}
                onChange={e => setEditRefMeia(e.target.value)}
                className="w-full border border-amber-300 rounded-md px-2.5 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-amber-400 bg-white"
                placeholder="Ref meia"
              />
            </div>
            <div className="flex-1 min-w-0">
              <label className="text-xs text-amber-600 block mb-1">Ref inteira</label>
              <input
                type="text"
          ...
          {showConfirm && (
            <div className="mt-3 p-2.5 bg-amber-100 border border-amber-300 rounded-md">
