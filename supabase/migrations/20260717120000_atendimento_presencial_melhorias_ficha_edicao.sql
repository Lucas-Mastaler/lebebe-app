-- Segunda revisao local das melhorias de Atendimento Presencial.
-- Nao aplicar no Supabase principal sem revisao e validacao em ambiente seguro.

alter table public.atendimento_presencial_atendimentos
  add column if not exists virada_cartao_dia smallint,
  add column if not exists virada_cartao_mes smallint;

alter table public.atendimento_presencial_criancas
  add column if not exists nome_nao_informado boolean not null default false;

alter table public.atendimento_presencial_atendimentos
  drop constraint if exists atendimento_presencial_atendimentos_virada_cartao_check,
  add constraint atendimento_presencial_atendimentos_virada_cartao_check
  check (
    (virada_cartao_dia is null and virada_cartao_mes is null)
    or (
      virada_cartao_dia is not null
      and virada_cartao_mes is not null
      and virada_cartao_mes between 1 and 12
      and virada_cartao_dia between 1 and 31
      and (
        (virada_cartao_mes in (1, 3, 5, 7, 8, 10, 12) and virada_cartao_dia <= 31)
        or (virada_cartao_mes in (4, 6, 9, 11) and virada_cartao_dia <= 30)
        or (virada_cartao_mes = 2 and virada_cartao_dia <= 29)
      )
    )
  );
...
alter table public.atendimento_presencial_atendimentos
  ...
  check (
    (
      ...
      and virada_cartao_dia is null
      and virada_cartao_mes is null
    )
    or (
      status = 'concluido'
      ...
      and (
        ...
        or (resultado_atendimento in ('nao', 'negociacao') and numero_lancamento is null)
      )
    )
  );

alter table public.atendimento_presencial_criancas
  drop constraint if exists atendimento_presencial_criancas_nome_nao_informado_check,
  add constraint atendimento_presencial_criancas_nome_nao_informado_check
  check (
    nome_nao_informado = false
    or (nome_nao_informado = true and (nome is null or btrim(nome) = ''))
  );

alter table public.atendimento_presencial_motivos
  drop constraint if exists atendimento_presencial_motivos_motivo_check,
  add constraint atendimento_presencial_motivos_motivo_check
  check (
    motivo = any (
      array[
        'qualidade_produto',
...
alter table public.atendimento_presencial_historico
  drop constraint if exists atendimento_presencial_historico_acao_check,
  add constraint atendimento_presencial_historico_acao_check
  check (acao in ('concluido', 'editado_concluido'));

alter table public.atendimento_presencial_historico
  drop constraint if exists atendimento_presencial_historico_snapshot_check,
  add constraint atendimento_presencial_historico_snapshot_check
  check (jsonb_typeof(snapshot) = 'object' and pg_column_size(snapshot) <= 4096);

create index if not exists idx_atendimento_presencial_atendimentos_virada_cartao
  on public.atendimento_presencial_atendimentos (
    ((virada_cartao_mes::integer * 100) + virada_cartao_dia::integer)
  )
  where status = 'concluido'
    and virada_cartao_dia is not null
    and virada_cartao_mes is not null;
...
begin
  ...
  select
    ...
    case
      when item.value->>'situacao' = 'gestacao' and item.value ? 'dataPrevistaNascimento'
...
revoke all on function public.atendimento_presencial_editar_concluido(uuid, integer, uuid, jsonb, integer)
  from public, anon, authenticated;
grant execute on function public.atendimento_presencial_editar_concluido(uuid, integer, uuid, jsonb, integer)
  to service_role;
